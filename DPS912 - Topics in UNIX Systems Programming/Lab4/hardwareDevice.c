...
static ssize_t hardware_device_read(struct file*, char __user*, size_t, loff_t*);
static long hardware_device_ioctl(struct file*, unsigned int, unsigned long);
...
static ssize_t hardware_device_read(struct file *filp, char __user *buf, size_t len, loff_t *off) {
    const size_t to_copy = BUF_LEN;
    if (copy_to_user(buf, buffer, to_copy)) return -1;

    printk(KERN_INFO "[HardwareDevice]: Read %zu bytes\n", to_copy);
    return to_copy;
}
...
static long hardware_device_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    switch(cmd) {
        case HARDWARE_DEVICE_HALT:
            printk(KERN_INFO, "[hardwareDevice]: Halted\n");
            is_halt = true;
            break;
        
        case HARDWARE_DEVICE_RESUME:
            printk(KERN_INFO, "[hardwareDevice]: Resumed\n");
            is_halt = false;
            break;
        
        case HARDWARE_DEVICE_GET_STATUS:
            const int device_state = is_halt ? 0 : 1;// have to convert to int
            if(copy_to_user((int *)arg, &device_state, sizeof(device_state))) return -EFAULT;
            break;

        default:
            printk(KERN_INFO, "[hardwareDevice]: Invalid IOCTL command\n");
            state = -1;
    }
