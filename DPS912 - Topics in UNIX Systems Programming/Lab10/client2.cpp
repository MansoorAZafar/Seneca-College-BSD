...
int main(void) {
    ...
    for(int i=0; i < NUM_MESSAGES && is_running; ++i) {
        std::cout << "[Client 2]: Waiting for Semaphore\n";
        semaphore.acquire(L10::MUTEX_SEM);
        std::cout << "[Client 2]: Successfully Got Semaphore\n";
        std::cout << "[Client 2]: Accessing Shared Memory\n";
        ...
        std::cout << "[Client 2]: Releasing Semaphore\n";
