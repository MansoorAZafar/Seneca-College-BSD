//client.cpp - client code with a read thread.
//
// 19-Nov-20  M. Watler         Created.

#include <arpa/inet.h>
#include <iostream>
#include <fstream>
#include <net/if.h>
#include <netinet/in.h>
#include <signal.h>
#include <string.h>
#include <sys/ioctl.h>
#include <sys/socket.h>
#include <sys/types.h>
#include <unistd.h>

bool is_running;
const int BUF_LEN=4096;

void *recv_func(void *arg);

int main(int argc, char *argv[]) {
    //Set up socket communications
    struct sockaddr_in addr;
    char buf[BUF_LEN];
    int len, ret;
    int fd,rc;

    if(argc<2) {
        std::cout << "usage: client <port number>\n";
	    return -1;
    }

    std::cout << "client(" << getpid() << "): running...\n";
    memset(&addr, 0, sizeof(addr));
    //Create the socket
    if ( (fd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
        std::cout << "client(" << getpid() << "): "<< strerror(errno) << "\n";
        exit(-1);
    }

    addr.sin_family = AF_INET;
    inet_pton(AF_INET, "127.0.0.1", &addr.sin_addr);
    addr.sin_port = htons(atoi(argv[1]));

    //Connect to the local socket
    if (connect(fd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        std::cout << "client(" << getpid() << "): " << strerror(errno) << "\n";
        close(fd);
        exit(-1);
    }

    pthread_t tid;
    ret = pthread_create(&tid, NULL, recv_func, &fd);
    if(ret!=0) {
        std::cout << "Cannot create recieve thread\n" << strerror(errno) << "\n";
        close(fd);
        return -1;
    }
    
    is_running=true;
    while(is_running) {
        sleep(1);
        len = sprintf(buf, "%d: It's not pining, it's passed on! This parrot is no more! It has ceased to be!\n\
It's expired and gone to meet its maker! This is a late parrot! It's a stiff!\n\
Bereft of life, it rests in peace! If you hadn't nailed it to the perch, it\n\
would be pushing up the daisies!\n\
It's rung down the curtain and joined the choir invisible. This is an ex-parrot!\n", getpid());
        
        std::cout << "client(" <<getpid() << "): write "<< len <<" bytes\n";
        ret = write(fd, buf, len);
        if(ret==-1) {
            std::cout << "client(" << getpid() << "): Write Error\n" << strerror(errno) << "\n";
        }
    }

    std::cout << "client(" << getpid() << "): stopping...\n";
    close(fd);
    return 0;
}

void *recv_func(void *arg) {
    int fd = *(int *)arg;
    char buf[BUF_LEN];
 
    while(is_running) {
        int len = read(fd,buf,BUF_LEN);
        std::cout << "client(" <<getpid() << "): read " << len << " bytes\n";
        
        if(len<0) sleep(1);
        else if(strcmp(buf, "Quit")==0) {
            std::cout << "client(" << getpid() << "): received request to quit\n";
            is_running = false;
        }
    }

    pthread_exit(NULL);
}