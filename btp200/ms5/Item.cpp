...
namespace sdds {
		...
		if (*this) {
			if (this->type == POS_LIST) {
				if (strLen(this->name) > 20) {
					...
					ostr << setw(7) << right << setprecision(2) << fixed <<this->price <<
						"| " << ((this->taxed) ? "X" : " ") << " |  " << this->qty << "|" << setw(9) << fixed << setprecision(2) << (this->cost() * this->qty) << "|";
				}
				else {
					ostr << left << setw(7) << this->SKU << "|" << left << setw(20) << this->name << "|" << setw(7) << right << setprecision(2) << fixed << this->price <<
						"| " << ((this->taxed) ? "X" : " ") << " |" << setw(4) << this->qty << "|" << setw(9) << setprecision(2) << fixed
						<< (this->cost() * this->qty) << "|" << left;
				}
			}
			else {
				ostr << "=============v" << endl <<
					"Name:        " << this->name << endl <<
					"Sku:         " << this->SKU << endl <<
					"Price:       " << setprecision(2) << fixed << this->price << endl;
				if (this->taxed) {
					ostr << "Price + tax: " << setprecision(2) << fixed << this->cost() << endl;
				}
				else {
	...
	std::istream& Item::read(std::istream& istr) {
		...
		if (choice == 'Y' || choice == 'y') { this->taxed = true; }
		else { this->taxed = false; }
		//--------qty---------------//
		cout << "Quantity" << endl << "> ";
		istr >> this->qty;
	...
	std::ofstream& Item::save(std::ofstream& ofstr) const {
		if (err) {
			...
			ofstr << this->itemType() << "," << this->SKU << "," << this->name << "," << fixed << setprecision(2) << this->price << "," << this->taxed
				<< "," << this->qty;
		}
		return ofstr;
	}
	...
	std::ostream& Item::bprint(std::ostream& ostr) const
	{
		if (strLen(this->name) > 20) {
			...
			ostr << "| " << left << setw(20) << this->name << "|" << setw(10) << right << setprecision(2) << 
				fixed << this->cost() << " |" << ((this->taxed) ? "  T  " : "     ") << "|" << endl;
	...
	{
		if (this->name == nullptr || rhs->name == nullptr) { return false; }
