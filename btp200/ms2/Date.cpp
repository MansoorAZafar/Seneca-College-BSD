    ...
    std::ostream& Date::display(std::ostream& ostr) const
    {
        ...
        case -4:
            ...
            if (this->flag) {
                ...
                ostr << this->error() << "(" << this->year << "/" << ((this->month < 10) ? "0" : "") << this->month << "/" << ((this->day < 10) ? "0" : "") << this->day << ", " <<
                    ((this->hour < 10) ? "0" : "") << ((this->hour < 0) ? 0 : this->hour) << ":" << ((this->minute < 10) ? "0" : "") <<
                    this->minute << ")";
            }
            break;
        case -5:
        ...
        */
        ...
        while (flag) {
            ...
            if (istr.fail()) {
                ...
                if (count == 1 && (((int)delim >= 65 && (int)delim <= 90) || ((int)delim >= 97 && (int)delim <= 122))) { m = 0; d = 0; h = 0; min = 0; flag = false; cinOK = false; rhs.err.operator=("Cannot read month entry"); }
                if (count == 2 && (((int)delim >= 65 && (int)delim <= 90) || ((int)delim >= 97 && (int)delim <= 122))) { d = 0; h = 0; min = 0; flag = false; cinOK = false; rhs.err.operator=("Cannot read day entry"); }
                if (count == 3 && (((int)delim >= 65 && (int)delim <= 90) || ((int)delim >= 97 && (int)delim <= 122))) { h = -1; min = 0; flag = false; cinOK = false; rhs.err.operator=("Cannot read hour entry"); }
                if (count == 4 && (((int)delim >= 65 && (int)delim <= 90) || ((int)delim >= 97 && (int)delim <= 122))) { min = -1; flag = false; cinOK = false; rhs.err.operator=("Cannot read minute entry"); }
            }
            else {
                switch (count) {
                case 0: y = tmp; break;
