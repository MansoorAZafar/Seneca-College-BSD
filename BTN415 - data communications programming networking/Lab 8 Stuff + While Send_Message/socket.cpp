...
packet create_packet(const std::string& str) {
    packet my_packet;
    std::strcpy(my_packet.name, "test");

    my_packet.size = str.length() + 1;
    ...
    std::strcpy(my_packet.letters, str.c_str());
    return my_packet;
...
int Server_TCP::send_message(std::string message) {
    ...
    while(totalBytesSent < sizeof(message)) {
        bytesSent = send(this->client_socket, message.c_str(), message.length(), 0);
        if(bytesSent < 0) std::cout << "could not send response\n";
...
int Server_TCP::send_packet(packet message) {
    ...
    int bytesSent = 0;
    int totalBytesSent = 0;
    while(totalBytesSent < sizeof(serialized)) {
        bytesSent = send(this->client_socket, serialized.data, serialized.length, 0);
        if(bytesSent < 0) std::cout << "could not send response\n";
        totalBytesSent += bytesSent; 
    }
    return 1;
