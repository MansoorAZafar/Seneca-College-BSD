...
void Run(int Index) {
    ...
    while (true) {
        ...
        //Assign x and y coord
        //Assign it to a string so we can easily grab the values + seperator
        std::string str{ RxBuffer };
        //Stops client err
        if (str.empty()) break;
        int x = std::stoi(str.substr(0, str.find(SEPERATOR) - 0));
        str.erase(0,str.find(SEPERATOR) + 1);
        int y = std::stoi(str.substr(0, str.find(SEPERATOR) - 0));
        
        std::string msg = "Client " + std::to_string(Index) + ": X=" + std::to_string(x) + ", Y=" + std::to_string(y);
        if (sizeof(RxBuffer) != 0);
        {
            std::cout << "From Thread " << Index << " : ";
            std::cout << RxBuffer << std::endl;
            send(ClientSockets[Index], msg.c_str(), msg.length(), 0);
        }
    }
