...
namespace sdds
{
	...
	int ProcessData::operator()(const std::string& target_file, double& avg, double& var)
	{
		...
		for (int i = 0; i <= this->num_threads; ++i)
		{
			...
			auto func2 = std::bind(computeVarFactor, this->data, this->p_indices[i], this->total_items, std::ref(avg), std::ref(var));
			avgThreads.push_back(std::thread(func));
			avgThreads[i].join();
			varThreads.push_back(std::thread(func2));
		}
		...
		for (int i = 0; i <= this->num_threads; ++i)
		{
			auto func = std::bind(computeVarFactor, this->data, this->p_indices[i], this->total_items, std::ref(avg), std::ref(var));
			varThreads.push_back(std::thread(func));
		}
