...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		//----added by me----//if it got to this point, File has to be open-able
		std::ifstream ifstr(argv[1]);
		size_t cnt{};
		std::string line{};
		while (ifstr)
	...
	for (auto index = 0u; index < library.size(); ++index)
	{
		fixPrice(library[index]);
	}
	...
	for (auto i = 3; i < argc; ++i)
	{
		...
		{
			std::cout << "** EXCEPTION: Unknown error but error regardless...\n";
		}
	}
	if (argc < 3) {
